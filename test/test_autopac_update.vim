...
" Tests for autopac#update()
func Test_autopac_update()
    call delete('pack', 'rf')
    call autopac#clear_pluglist()
    call autopac#init()

    " autopac#update() with hooks using strings.
    call autopac#add('k-takata/minpac', {'type': 'opt',
                \ 'do': 'let g:post_update = 1'})
    let g:post_update = 0
    let g:finish_update = 0
    call autopac#update({'do': 'let g:finish_update = 1'})
    ...
    " autopac#update() with hooks using funcrefs.
    let l:post_update = 0
    call autopac#add('k-takata/hg-vim', {'type':'start', 'do': {hooktype, name -> [
    ...
    call autopac#update({'do': {hooktype, updated, installed -> [
